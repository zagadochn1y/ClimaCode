...
<p class="date">Issued on ${certDate}</p>
    ...
    const aiResp = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `You are a sustainability expert. Generate a detailed analysis for a website carbon audit report. Include: Overview, Key Findings, Detailed Recommendations, and Next Steps. Keep it professional and under 500 words.\n\nWebsite: ${url}\nCO2 per view: ${co2PerView}g\nCO2 per year: ${co2PerYear}kg\nSustainability Score: ${sustainabilityScore}\nBreakdown: Images ${breakdown?.images}%, CSS ${breakdown?.css}%, JavaScript ${breakdown?.javascript}%, Other ${breakdown?.other}%\nRecommendations: ${recommendations?.join("; ")}`,
            }],
          }],
        }),
      }
    ...
    const analysisContent = aiData.candidates?.[0]?.content?.parts?.[0]?.text ?? "Analysis unavailable.";
