...
import GlobalEmissions from "@/components/GlobalEmissions";
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 relative">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="flex-1">
            ...
            <div className="flex gap-3">
              <Link to="/climascan">
                <Button size="lg" className="rounded-full px-8 text-base font-semibold neon-glow-subtle">
              ...
              <Button size="lg" variant="outline" className="rounded-full px-8 text-base font-semibold"
                onClick={() => document.getElementById("global-emissions")?.scrollIntoView({ behavior: "smooth" })}>
                {t("hero.learn")}
              </Button>
            </div>
          </motion.div>
      ...
      <div id="global-emissions">
        <GlobalEmissions />
      </div>

      {/* Stats */}
      <section className="bg-primary text-primary-foreground py-12 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-primary via-primary to-primary animate-gradient" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} viewport={{ once: true }}>
              <Card className="text-center h-full border-2 border-border hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5">
                <CardContent className="pt-8 pb-6 px-6 flex flex-col items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-accent flex items-center justify-center neon-glow-subtle">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {howItWorks.map((item, i) => (
              <motion.div key={item.step} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }} viewport={{ once: true }}>
                <Card className="h-full border-2 border-border hover:border-primary/40 transition-all duration-300 relative overflow-hidden hover:shadow-lg hover:shadow-primary/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <motion.div key={product.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} viewport={{ once: true }}>
              <Card className="h-full border-2 transition-all duration-300 border-primary/30 hover:border-primary hover:shadow-xl hover:shadow-primary/10 neon-border">
                <CardContent className="pt-8 pb-6 px-6 flex flex-col h-full">
                  <div className="w-12 h-12 rounded-xl bg-accent flex items-center justify-center mb-4 neon-glow-subtle">
                    <product.icon className="h-6 w-6 text-primary" />
                  </div>
                  <h3 className="font-bold text-lg mb-2">{product.title}</h3>
                  <p className="text-muted-foreground text-sm flex-1 mb-4">{product.description}</p>
                  <Link to={product.link}>
                    <Button variant="outline" className="rounded-full w-full hover:bg-primary hover:text-primary-foreground transition-colors">
      ...
      <section className="container py-16">
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {team.map((member) => (
            <Card key={member.name} className="w-56 overflow-hidden border-2 border-primary/30 hover:border-primary transition-colors">
