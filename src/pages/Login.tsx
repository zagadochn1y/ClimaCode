    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-2">
        <CardHeader className="text-center">
          ...
          <CardTitle>{isSignUp ? t("login.create") : t("login.welcome")}</CardTitle>
          <CardDescription>{isSignUp ? t("login.signupDesc") : t("login.signinDesc")}</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ...
            <button onClick={() => { setIsSignUp(!isSignUp); setErrors({}); }} className="text-primary font-medium hover:underline">
              {isSignUp ? t("login.submit.login") : t("login.submit.signup")}
            </button>
          </p>
        </CardContent>
      </Card>
