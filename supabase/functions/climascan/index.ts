...
serve(async (req) => {
  ...
  try {
              ...
              contents: [
                {
                  ...
                  parts: [
                    {
                      text: `You are a web sustainability expert.
    Return exactly 4 recommendations as a JSON array of strings.
    Format each like: "Title — Description".
    ...
    return new Response(JSON.stringify({
      co2PerView, co2PerYear, sustainabilityScore, breakdown, recommendations, bytes, countryFactor,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
